...
namespace UnityEngine.UI.Extensions
{
    ...
    public class AutoCompleteComboBox : MonoBehaviour
    {
        ...
        //charles' edits below; mostly UI "improvements"
        /* Initialize() moved to Start() to allow for ProgramManager's Awake()
        public void Awake()
        {
            Initialize();
        }
        */
        ...
        public float dropdownOffset2 = 100 - 73f; //dropdown length
        ...
        void after() {
            ...
            for (int i = 0; i < itemList.transform.childCount; i++) {
                itemList.transform.GetChild(i).GetComponent<RectTransform>().sizeDelta = widthHeight;
            }

            scrollPanel.transform.position = new Vector3(scrollPanel.transform.position.x, scrollPanel.transform.position.y + .04f, scrollPanel.transform.position.z);
        }
        ...
        public void setAvailableOptions(List<string> a) {
            AvailableOptions = a;
        }
        ...
        private void OnItemClicked(string item)
        {
            ...
            Text = item;
            _mainInput.text = Char.ToUpper(item[0]) + item.Substring(1);
        ...
        private void RedrawPanel()
        {
            ...
            dropdownHeight = _rectTransform.sizeDelta.y * Mathf.Min(_itemsToDisplay, _panelItems.Count) + DropdownOffset;
            //bottom statement added to make dropdown smaller --char
            if (_panelItems.Count > 3) {
                dropdownHeight = _rectTransform.sizeDelta.y * Mathf.Min(_itemsToDisplay, _panelItems.Count) + DropdownOffset - dropdownOffset2;
            }
